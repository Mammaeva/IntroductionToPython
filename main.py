...
def print_weather(dict_weather, message):
    bot.send_message(message.from_user.id, f'Weather Bot желает отличного дня и солнечного настроения!'
                                           f' Температура сейчас {dict_weather["сейчас"]["temp"]}!'
                                           f' На небе {dict_weather["сейчас"]["sky"]}.\n'
                                           f' Температура через три часа {dict_weather["через3ч"]["temp"]}!'
                                           f' На небе {dict_weather["через3ч"]["sky"]}.')
...
def get_text_messages(message):
    ...
    if message.text.lower() == 'привет' or message.text.lower() == 'здорова':
        bot.send_message(message.from_user.id,
                         ...
                         f' Напиши слово "погода" и я напишу погоду в твоем'
